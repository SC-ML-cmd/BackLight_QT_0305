 ...
 *@Mat&                 原图像,利用引用返回对应的凸集图像
 *@修改时间：             2021/03/16
 *=======================================================*/
void convexSetPretreatment(Mat& _src){
    //空洞预处理 针对于主相机
    Mat src_copy = _src.clone();
...
*@修改时间：		         2020年03月16日
*@备注说明                添加刘海水滴预处理阶段，使透视变换更准确
=========================================================*/
bool f_MainCam_PersTransMatCal(InputArray _src, int border_white, int border_biankuang, Mat* Mwhite, Mat* Mbiankuang, Mat* M_white_abshow, int ID, QString ScreenType_Flag, int leftRightWhiteFlag)
{
...
bool Dead_light(Mat white, Mat* mresult, QString* causecolor)
{
    ...
    double meanGray = mean(maskR1, maskR1_Binary)[0];
    if (meanGray <= 50)
    {
    ...
    else
    {
        ...
        meanStdDev(maskR1, Mean, stdDev, maskR1_Binary);
        double stddev1 = stdDev.at<double>(0, 0);
        double MeanR1 = Mean.at<double>(0, 0);
        if (MeanR1 >= 130)
        {
            if (abs(stddev1) <= 7)
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 10, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev1) <= 13)
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 20, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev1) <= 20)
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 28, 255, CV_THRESH_BINARY);
            }
            else
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 35, 255, CV_THRESH_BINARY);
            }
        }
        else
        {
            if (abs(stddev1) <= 7)
            {
                threshold(maskR1, maskR1_Binary, MeanR1, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev1) <= 13)
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 10, 255, CV_THRESH_BINARY);
            ...
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 18, 255, CV_THRESH_BINARY);
            }
            else
            {
                threshold(maskR1, maskR1_Binary, MeanR1 - 25, 255, CV_THRESH_BINARY);
            }
        }
    }
    ...
    if (meanGray <= 50)
    {
        maskR2_Binary = ~Mat::zeros(maskR2.size(), maskR2.type());
    }
    else
    {
        if (meanGray >= 130)
            threshold(maskR2, maskR2_Binary, meanGray - 45, 255, CV_THRESH_BINARY);
        else
            threshold(maskR2, maskR2_Binary, meanGray - 35, 255, CV_THRESH_BINARY);
        meanStdDev(maskR2, Mean, stdDev, maskR2_Binary);
        double stddev2 = stdDev.at<double>(0, 0);
        double MeanR2 = Mean.at<double>(0, 0);
        if (MeanR2 >= 130)
        {
            if (abs(stddev2) <= 7)
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 10, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev2) <= 13)
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 20, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev2) <= 20)
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 28, 255, CV_THRESH_BINARY);
            }
            else
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 35, 255, CV_THRESH_BINARY);
            }
        }
        else
        {
            if (abs(stddev2) <= 7)
            {
                threshold(maskR2, maskR2_Binary, MeanR2, 255, CV_THRESH_BINARY);
            }
            else if (abs(stddev2) <= 13)
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 10, 255, CV_THRESH_BINARY);
            ...
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 18, 255, CV_THRESH_BINARY);
            }
            else
            {
                threshold(maskR2, maskR2_Binary, MeanR2 - 25, 255, CV_THRESH_BINARY);
            }
        }
    }
    ...
    for (vector<int>::size_type i = 0; i < contours.size(); i++)
    {
        ...
        if (area > SDPara->areaDownLimit && area < SDPara->areaUpperLimit)
        {
            ...
            if (max(w / h, h / w) <= SDPara->lengthWidthRadio) //wsc将长宽比 由4--> 6.5 -->7 -->13 0313将长宽比修改为--> 8
            {
                ...
                if (intensity <= 18 && mean_in_gray <= 80 && mean_out_gray <= 80)
                {
...
bool Mura_Decter(Mat imageGray, Mat *mresult, QString *causecolor)
{
    ...
    for (vector<int>::size_type i = 0; i < contours.size(); i++)
    {
        ...
        if (area >= 100 && area < 60000)
        {
            ...
            //长宽比排除
            if (radio > 7 && (Y_1 >= th_result.rows - 40 || Y_2 <= 40))   //日期：2021/3/2 6504: 10 -> 7
            {
                continue;
            }
            ...
            else if (differ < 6.4 && differ >= 0)
            {
                if (w <= h)
                {
                    ...
                    threshold(tempGray_Width, temp_Binary2, mean_All + 40, 255, CV_THRESH_BINARY);
                ...
                else if (w > h)
                {
                    ...
                    threshold(tempGray_Height, temp_Binary2, mean_All + 40, 255, CV_THRESH_BINARY);
...
======================================================================*/
   bool Shifting(Mat white, Mat* mresult, QString* causecolor, int num, Mat& left_white, Mat& right_white)
   {
       ...
       Mat ad_result, th1, th2, th3, th4, ImageBinary;
...
* 备注说明：2020年9月25日修改
 =========================================================*/
bool Brightedge(Mat src_white, Mat photomainwhite,Mat* Mwhite, QString* causecolor)
{
...
bool white_defect1(Mat white,Mat mainfilter,Mat leftfilter,Mat rightfilter,Mat ceguang,Mat ceguangleft,Mat ceguangright,int num,Mat biankuangfilter)
{
    ...
    catch (const std::exception& e)
    {
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1(current_date + "C++划伤算法异常 +++++++++++++++++++++++++++");
        debug_msg1(e.what());
    }
    catch (cv::Exception& e)
    {
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1(current_date + "CV划伤算法异常 +++++++++++++++++++++++++++");
        debug_msg1(e.what());
    }
    catch(...){
        current_date_time =QDateTime::currentDateTime();
...
bool white_defect2(Mat white,Mat mainfilter,Mat ceguang,Mat biankuangfilter,Mat photomainwhite)
{
    ...
    QDateTime current_date_time =QDateTime::currentDateTime();
    QString current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
    debug_msg1("刚进入检测子线程2++"+current_date);

    //==========================爆灯检测==============================
    if(result==false&&boom_light_Checked)
    {
        result=boom_light(mainfilter,&Mresult_2_white,&causeColor_2_white);

        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1("爆灯检测完毕"+current_date);
    }
    ...
    try{
        ...
        {
            result = Brightedge(biankuangfilter,photomainwhite,&Mresult_2_white, &causeColor_2_white);

            current_date_time =QDateTime::currentDateTime();
            current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
            debug_msg1("亮边检测完毕"+current_date);
    ...
    //                result=Brightedge(CeR1_Enlarge_Fliter,ceguangright_enlarge,&Mresult_2_white, &causeColor_2_white,1);
        ...
        if(result==false&&WhiteDot_Checked)
        {
            ...
            current_date_time =QDateTime::currentDateTime();
            current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
            debug_msg1("白点检测完毕"+current_date);
    ...
    catch (const std::exception& e)
    {
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1(current_date + "亮边|白点算法异常 +++++++++++++++++++++++++++");
        debug_msg1(e.what());
    }
    catch (cv::Exception& e)
    {
...
bool white_defect3(Mat white_abshow1,Mat mainfilter,Mat leftfilter,Mat rightfilter,Mat ceguangleft,Mat ceguangright, Mat left_mask, Mat right_mask, int num)
{
    ...
    QDateTime current_date_time =QDateTime::currentDateTime();
    QString current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
    debug_msg1("刚进入检测子线程3++"+current_date);
    //==========================死灯检测==============================
    if(result==false&&LampEye_Checked)
    {
//       qDebug() <<  QString(QLatin1String("DeadLightInput：")) + QString::number(mean(mainfilter)[0]);
//        result=LampEye(mainfilter, 5, 2, 15, 1, 6, 0.1, 8, 35, 0.1,0.12, 20, 9, 9, &Mresult_3_white, &causeColor_3_white);


        result = Dead_light(mainfilter, &Mresult_3_white,&causeColor_3_white);
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1("死灯检测完毕"+current_date);
    }
...
//        result=Abshow_White_Detect(white_abshow1,&Mresult_3_white,&causeColor_3_white);//主黑白相机白底显异检测
        ...
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1("移位检测完毕"+current_date);
    }
...
//        result = LessMaterial(mainfilter, &Mresult_3_white, &causeColor_3_white);
    ...
    if(result==false&&mura_Checked)
    {
        ...
        current_date_time =QDateTime::currentDateTime();
        current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        debug_msg1("白印检测完毕"+current_date);
...
* 其他：
======================================================================*/
bool white_defect(Mat white,Mat whiteleft,Mat whiteright,Mat whitecolor,Mat ceguang,Mat back,Mat ceguangback,Mat front,Mat ceguangfront,Mat ceguangleft,Mat ceguangright,int num,Mat white_abshow1,Mat CeL1_Enlarge, Mat CeR1_Enlarge,Mat white_biankuang,Mat ceguangleft_enlarge,Mat ceguangright_enlarge,Mat photomainwhite)
{
    ...
    if(result==false&&Brightedge_Checked)
    {
        ...
        case 1:
        {
            result=Brightedge(CeL1_Enlarge_Fliter,photomainwhite,&Mresult_1_white, &causeColor_1_white);
            if(!result)
                result=Brightedge(CeR1_Enlarge_Fliter,photomainwhite,&Mresult_1_white, &causeColor_1_white);
            break;
        }
        case 2:result=Brightedge(CeR1_Enlarge,photomainwhite,&Mresult_1_white, &causeColor_1_white);
            break;
        }
    }
...
//        }
    ...
    if(result==false&&boom_light_Checked)
    {
        switch(num)
        {
        case 1:result=boom_light(mainfilter,&Mresult_1_white,&causeColor_1_white);
            break;
        case 2:result=boom_light(mainfilter,&Mresult_2_white,&causeColor_2_white);
            break;
        }
    }
