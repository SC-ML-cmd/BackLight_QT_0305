...
int HikvisionSDK::connectCamera(int id)
{
    ...
    {
        qDebug()<<"打开设备成功"<<endl;
        if(qStr=="00D58026006")//左彩色相
        {
            m_hDevHandle_1=m_hDevHandle;
            return 0;
        }
        if(qStr=="00C61651293")// 主彩色相机
        {
            m_hDevHandle_2=m_hDevHandle;
            return 0;
        }
        if(qStr=="00C77088784") //右彩色相机
        {
...
cv::Mat HikvisionSDK::saveImage7(int id)
{
    ...
    catch (const std::exception& e)
    {
        ...
        MainWindow::debug_msg(current_date + "C++: saveImage7()异常 +++++++++++++++++++++++++++");
        MainWindow::debug_msg(e.what());
    }
    catch (cv::Exception& e)
    {
        QDateTime current_date_time =QDateTime::currentDateTime();
        QString current_date = current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        MainWindow::debug_msg(current_date + "CV: saveImage7()异常 +++++++++++++++++++++++++++");
        MainWindow::debug_msg(e.what());
    }
    catch(...){
        QDateTime current_date_time =QDateTime::currentDateTime();
        QString current_date = current_date_time.toString("yyyy.MM.dd hh:mm:ss.zzz");
        MainWindow::debug_msg(current_date + "saveImage7()异常 +++++++++++++++++++++++++++");
    }
}
